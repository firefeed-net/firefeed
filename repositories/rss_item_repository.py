        ...
        async with self.db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                try:
                    ...
                    raise DatabaseException(f"Failed to get category ID by name {category_name}: {str(e)}")
        ...
        async with self.db_pool.acquire() as conn:
            async with conn.cursor() as cur:
                try:
                    ...
                    raise DatabaseException(f"Failed to get source ID by alias {source_alias}: {str(e)}")
