...
async def get_news(
    ...
    telegram_published: Optional[bool] = Query(
        None, description="Фильтр по статусу публикации в Telegram (true/false)"
    ),
    from_date: Optional[int] = Query(
        None,
        description="Фильтр по дате публикации (timestamp в миллисекундах). Возвращает новости, опубликованные после этой даты.",
    ),
    search_phrase: Optional[str] = Query(
        None, alias="searchPhrase", description="Строка для поиска по заголовку и содержанию на языке отображения"
    ),
    include_all_translations: Optional[bool] = Query(
        None,
        description="Вернуть переводы для всех языков (дороже по ресурсам). Автоматически true, если display_language не указан",
    ),
    cursor_published_at: Optional[int] = Query(
        None, description="Курсор пагинации: published_at (ms) для keyset-пагинации"
    ),
    ...
    if pool is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Ошибка подключения к базе данных"
        )
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create verification code"
                )
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            try:
                ...
                await cur.execute("UPDATE users SET is_active = TRUE WHERE id = %s", (user["id"],))
...
async def update_user_categories(
    category_update: models.UserCategoriesUpdate, current_user: dict = Depends(get_current_active_user)
):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            if invalid_ids:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid category IDs: {list(invalid_ids)}"
                )
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update user categories"
        )
...
async def unlink_telegram_account(current_user: dict = Depends(get_current_active_user)):
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to unlink Telegram account"
        )
        ...
        async with conn.cursor() as cur:
            ...
            if result:
                return models.TelegramLinkStatusResponse(
                    is_linked=True, telegram_id=result[0], linked_at=result[1].isoformat() if result[1] else None
                )
            else:
...
async def update_user_rss_feed(
    feed_id: int, feed_update: models.UserRSSFeedUpdate, current_user: dict = Depends(get_current_active_user)
):
